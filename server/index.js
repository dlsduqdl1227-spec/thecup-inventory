globalThis.__VINEXT_LAZY_CHUNKS__ = ["assets/EduSystemApp-fu3E7Hen.js","assets/layout-segment-context-DnzLEFy1.js"];
...
function getDraftSecret() {
	return "8274ad0a-9f1f-4b20-a031-57979a6cf20b";
}
/**
...
var NextURL = class NextURL {
	...
	*/
	get buildId() {
		return "ec3b4f77-478f-49f8-8ebc-60b092febc60";
	}
};
...
*/
function appIsrCacheKey(pathname, suffix, buildId = "ec3b4f77-478f-49f8-8ebc-60b092febc60") {
...
function createAppPageArtifactCompatibility(element, routePattern) {
	...
	return createArtifactCompatibilityEnvelope({
		...
		deploymentVersion: "ec3b4f77-478f-49f8-8ebc-60b092febc60",
		rootBoundaryId
	});
}
/**
...
var EduSystemApp = /* @__PURE__ */ registerClientReference(() => {
	throw new Error("Unexpectedly client reference export 'EduSystemApp' is called on server");
}, "32a1558e3d99", "EduSystemApp");
//#endregion
//#region app/page.tsx
...
function Home() {
	return /* @__PURE__ */ (0, import_jsx_runtime_react_server.jsx)(EduSystemApp, {});
}
...
var title = "더컵에듀 시스템";
var description = "더컵에듀의 재고, 매출, 영수증, 로스팅 프로파일을 연결한 통합 운영 시스템";
...
async function initializeDatabase() {
	...
	const financeSeedStatements = monthlySeeds.map((row) => db.prepare(`INSERT OR IGNORE INTO monthly_finance
          (year, month, revenue, baseline_expense, note, source)
         VALUES (?, ?, ?, ?, ?, ?)`).bind(row.year, row.month, row.revenue, row.expense, row.note ?? "", `더컵에듀 연매출 분석표 KPI - ${row.year}년 연매출.csv`));
	...
	].map(([category, name, unit, reorder]) => db.prepare(`INSERT INTO inventory_items (category, name, unit, quantity, reorder_level)
         SELECT ?, ?, ?, 0, ?
         WHERE NOT EXISTS (SELECT 1 FROM inventory_items WHERE name = ?)`).bind(category, name, unit, reorder, name));
	await db.batch([...financeSeedStatements, ...inventorySeedStatements]);
	await db.prepare(`UPDATE inventory_items
       SET name = '더컵 볶은 원두', updated_at = CURRENT_TIMESTAMP
       WHERE category = 'roasted' AND name = '더컵 로스팅 원두'`).run();
}
async function ensureStaffPermissionColumns(db) {
	const columns = await db.prepare("PRAGMA table_info(staff)").all();
	...
	for (const [, statement] of missing) await db.prepare(statement).run();
	if (missing.length > 0) await db.prepare(`UPDATE staff
         SET can_finance = 1, can_inventory = 1, can_roasting = 1
         WHERE role IN ('admin', 'employee')`).run();
}
async function audit(actorId, action, entityType, entityId = "", detail = "") {
	await getD1().prepare(`INSERT INTO audit_logs (actor_id, action, entity_type, entity_id, detail)
       VALUES (?, ?, ?, ?, ?)`).bind(actorId, action, entityType, entityId, detail.slice(0, 500)).run();
}
//#endregion
//#region lib/auth.ts
var SESSION_COOKIE = "thecup_session";
var SESSION_DAYS = 30;
var LOGIN_WINDOW_MINUTES = 15;
var MAX_LOGIN_ATTEMPTS = 5;
function normalizePhone(value) {
	const digits = value.replace(/\D/g, "");
	if (digits.length < 10 || digits.length > 11 || !digits.startsWith("01")) throw new Error("휴대폰 번호를 정확히 입력해 주세요.");
	return digits;
}
...
async function createSession(staffId) {
	...
	await getD1().prepare("INSERT INTO sessions (token_hash, staff_id, expires_at) VALUES (?, ?, ?)").bind(tokenHash, staffId, expiresAt).run();
	return {
		token,
		expiresAt
	};
...
async function getSessionUser(request) {
	...
	const row = await getD1().prepare(`SELECT s.id, s.name, s.role,
              s.can_finance AS canFinance,
              s.can_inventory AS canInventory,
              s.can_roasting AS canRoasting
       FROM sessions x
       JOIN staff s ON s.id = x.staff_id
       WHERE x.token_hash = ? AND x.expires_at > ? AND s.active = 1`).bind(await sha256(token), (/* @__PURE__ */ new Date()).toISOString()).first();
	return row ? normalizeSessionUser(row) : null;
}
async function requirePermission(request, permission) {
	const user = await requireUser(request);
	if (!(permission === "finance" ? user.canFinance : permission === "inventory" ? user.canInventory : user.canRoasting)) throw new AuthError("이 메뉴를 사용할 권한이 없습니다.", 403);
	return user;
}
...
async function requireUser(request, roles) {
	...
	if (roles && !roles.includes(user.role)) throw new AuthError("이 기능을 사용할 권한이 없습니다.", 403);
	return user;
}
function sessionCookie(token, expiresAt) {
	return `${SESSION_COOKIE}=${token}; Path=/; HttpOnly; SameSite=Strict; Expires=${new Date(expiresAt).toUTCString()}; Secure`;
}
...
function getSessionSecret() {
	...
	if (!value || value.length < 24) throw new Error("세션 보안 키가 올바르게 설정되지 않았습니다.");
	return value;
}
function verifyCode(input, expected) {
	if (input.length !== expected.length) return false;
	let mismatch = 0;
	for (let index = 0; index < input.length; index += 1) mismatch |= input.charCodeAt(index) ^ expected.charCodeAt(index);
	return mismatch === 0;
}
async function assertLoginAllowed(identifierHash) {
	const db = getD1();
	const now = Date.now();
	const row = await db.prepare("SELECT window_start, attempt_count FROM login_attempts WHERE identifier_hash = ?").bind(identifierHash).first();
	if (!row) return;
	if (now - new Date(row.window_start).getTime() < LOGIN_WINDOW_MINUTES * 6e4 && row.attempt_count >= MAX_LOGIN_ATTEMPTS) throw new AuthError("로그인 시도가 많습니다. 15분 후 다시 시도해 주세요.", 429);
}
async function recordLoginFailure(identifierHash) {
	const db = getD1();
	const now = /* @__PURE__ */ new Date();
	const row = await db.prepare("SELECT window_start, attempt_count FROM login_attempts WHERE identifier_hash = ?").bind(identifierHash).first();
	if (!row || now.getTime() - new Date(row.window_start).getTime() >= LOGIN_WINDOW_MINUTES * 6e4) await db.prepare(`INSERT INTO login_attempts (identifier_hash, window_start, attempt_count)
         VALUES (?, ?, 1)
         ON CONFLICT(identifier_hash) DO UPDATE SET window_start = excluded.window_start, attempt_count = 1`).bind(identifierHash, now.toISOString()).run();
	else await db.prepare("UPDATE login_attempts SET attempt_count = attempt_count + 1 WHERE identifier_hash = ?").bind(identifierHash).run();
}
async function clearLoginFailures(identifierHash) {
	await getD1().prepare("DELETE FROM login_attempts WHERE identifier_hash = ?").bind(identifierHash).run();
}
...
function jsonError(error) {
	...
	if (message.includes("inventory_quantity_negative")) return Response.json({ error: "다른 작업으로 재고가 변경됐습니다. 현재 수량을 새로고침한 뒤 다시 입력해 주세요." }, { status: 409 });
	if (/(D1_ERROR|SQLITE|constraint failed|database|R2Bucket)/i.test(message)) {
		console.error("Internal persistence error", error);
		return Response.json({ error: "저장 중 일시적인 오류가 발생했습니다. 잠시 후 다시 시도해 주세요." }, { status: 500 });
	}
...
function assertSameOrigin(request) {
	...
	if (!origin) return;
	const requestUrl = new URL(request.url);
	if (new URL(origin).host !== requestUrl.host) throw new AuthError("허용되지 않은 요청입니다.", 403);
}
function textValue(value, label, maxLength = 200) {
	const text = typeof value === "string" ? value.trim() : "";
	if (!text) throw new Error(`${label}을(를) 입력해 주세요.`);
	if (text.length > maxLength) throw new Error(`${label}이(가) 너무 깁니다.`);
	return text;
}
function optionalText(value, maxLength = 1e3) {
	const text = typeof value === "string" ? value.trim() : "";
...
function positiveNumber(value, label) {
	...
	if (!Number.isFinite(number) || number <= 0) throw new Error(`${label}은(는) 0보다 커야 합니다.`);
	return number;
}
function nonNegativeNumber(value, label) {
	const number = Number(value);
	if (!Number.isFinite(number) || number < 0) throw new Error(`${label}은(는) 0 이상이어야 합니다.`);
	return number;
}
function integerAmount(value, label = "금액") {
	const amount = Math.round(positiveNumber(value, label));
	if (amount > 1e9) throw new Error(`${label}이(가) 허용 범위를 넘었습니다.`);
	return amount;
}
function isoDate(value) {
	...
	if (date.getUTCFullYear() !== year || date.getUTCMonth() !== month - 1 || date.getUTCDate() !== day) throw new Error("존재하지 않는 날짜입니다.");
	return text;
}
//#endregion
//#region app/api/auth/bootstrap/route.ts
var route_exports$13 = /* @__PURE__ */ __exportAll({ POST: () => POST$9 });
async function POST$9(request) {
	try {
		...
		if (Number(count?.count ?? 0) > 0) return Response.json({ error: "초기 관리자 등록이 이미 완료되었습니다." }, { status: 409 });
		const payload = await request.json();
		if (!verifyCode(textValue(payload.code, "초기 관리자 코드", 100), getBootstrapCode())) return Response.json({ error: "초기 관리자 코드가 올바르지 않습니다." }, { status: 403 });
		const name = textValue(payload.name, "이름", 40).replace(/\s+/g, " ");
		const phone = normalizePhone(String(payload.phone ?? ""));
		const result = await db.prepare(`INSERT INTO staff
          (name, phone_hash, phone_last4, role, can_finance, can_inventory, can_roasting)
         SELECT ?, ?, ?, 'admin', 1, 1, 1
         WHERE NOT EXISTS (SELECT 1 FROM staff)`).bind(name, await phoneHash(phone), phone.slice(-4)).run();
		if (Number(result.meta.changes ?? 0) !== 1) return Response.json({ error: "초기 관리자 등록이 이미 완료되었습니다." }, { status: 409 });
...
//#endregion
//#region app/api/auth/login/route.ts
var route_exports$12 = /* @__PURE__ */ __exportAll({ POST: () => POST$8 });
async function POST$8(request) {
	try {
		...
		const payload = await request.json();
		const name = textValue(payload.name, "이름", 40).replace(/\s+/g, " ");
		const hashedPhone = await phoneHash(normalizePhone(String(payload.phone ?? "")));
		const identifierHash = hashedPhone;
		await assertLoginAllowed(identifierHash);
		const row = await getD1().prepare(`SELECT id, name, role,
                can_finance AS canFinance,
                can_inventory AS canInventory,
                can_roasting AS canRoasting
         FROM staff
         WHERE name = ? AND phone_hash = ? AND active = 1`).bind(name, hashedPhone).first();
		if (!row) {
			await recordLoginFailure(identifierHash);
			return Response.json({ error: "등록된 이름과 휴대폰 번호가 일치하지 않습니다." }, { status: 401 });
		}
...
//#endregion
//#region app/api/auth/logout/route.ts
var route_exports$11 = /* @__PURE__ */ __exportAll({ POST: () => POST$7 });
...
//#endregion
//#region app/api/auth/status/route.ts
var route_exports$10 = /* @__PURE__ */ __exportAll({ GET: () => GET$4 });
async function GET$4(request) {
	try {
		await ensureDatabase();
		const row = await getD1().prepare("SELECT COUNT(*) AS count FROM staff").first();
		const user = await getSessionUser(request);
...
//#endregion
//#region app/api/dashboard/route.ts
var route_exports$9 = /* @__PURE__ */ __exportAll({ GET: () => GET$3 });
async function GET$3(request) {
	try {
		await ensureDatabase();
		const user = await requireUser(request);
		const db = getD1();
		const finance = user.canFinance ? await db.prepare(`SELECT m.year, m.month,
               ...
               GROUP BY m.id
               ORDER BY m.year, m.month`).all() : { results: [] };
		const inventory = await db.prepare(`SELECT id, category, name, unit, quantity, reorder_level AS reorderLevel,
		...
		const movementSql = !user.canInventory ? `SELECT m.id, m.movement_type AS movementType, m.quantity,
           ...
           ORDER BY m.id DESC LIMIT 30` : `SELECT m.id, m.movement_type AS movementType, m.quantity,
                  m.movement_date AS movementDate, m.note, m.class_name AS className,
                  m.cost_amount AS costAmount, m.receipt_key IS NOT NULL AS hasReceipt,
                  i.name AS itemName, i.unit, s.name AS createdByName
           FROM inventory_movements m
		...
		const movements = !user.canInventory ? await db.prepare(movementSql).bind(user.id).all() : await db.prepare(movementSql).all();
		const transactions = user.canFinance ? await db.prepare(`SELECT t.id, t.kind, t.category, t.amount,
               ...
               ORDER BY t.id DESC LIMIT 40`).all() : { results: [] };
		const profiles = user.canRoasting ? await db.prepare(`SELECT p.id, p.bean_name AS beanName, p.origin, p.process,
               ...
               ORDER BY p.id DESC`).all() : { results: [] };
		return Response.json({
			user,
...
//#endregion
//#region app/api/finance/route.ts
...
async function POST$6(request) {
	try {
		...
		const result = await getD1().prepare(`INSERT INTO finance_transactions
          (kind, category, amount, transaction_date, description, created_by)
         VALUES (?, ?, ?, ?, ?, ?)`).bind(kind, category, amount, transactionDate, description, user.id).run();
...
//#endregion
//#region app/api/inventory/route.ts
...
async function POST$5(request) {
	try {
		...
		if (action === "create_item") {
           ...
           VALUES (?, ?, ?, 0, ?, ?)`).bind(category, name, unit, reorderLevel, user.id).run();
			const id = Number(result.meta.last_row_id);
		...
		if (![
			...
			"out",
			"adjust"
		].includes(movementType)) throw new Error("입고·사용·실사 조정 중 하나를 선택해 주세요.");
		const inputQuantity = movementType === "adjust" ? nonNegativeNumber(payload.quantity, "실사 수량") : positiveNumber(payload.quantity, "수량");
		const item = await db.prepare("SELECT id, name, quantity, unit FROM inventory_items WHERE id = ? AND active = 1").bind(itemId).first();
		...
		const movement = db.prepare(`INSERT INTO inventory_movements
         ...
         VALUES (?, ?, ?, ?, ?, ?)`).bind(itemId, movementType, delta, movementDate, note, user.id);
		const update = db.prepare("UPDATE inventory_items SET quantity = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(nextQuantity, itemId);
...
//#endregion
//#region app/api/inventory/class-use/route.ts
...
async function POST$4(request) {
	try {
		...
		const note = optionalText(payload.note, 300);
		if (milkQuantity <= 0 && beanQuantity <= 0) throw new Error("우유 또는 원두 사용량을 입력해 주세요.");
		const db = getD1();
		const items = await db.prepare(`SELECT id, name, category, quantity, unit
         FROM inventory_items WHERE active = 1 AND (category = 'milk' OR id = ?)`).bind(Number.isInteger(beanItemId) ? beanItemId : -1).all();
		...
		if (beanQuantity > 0 && (!beanItem || !["roasted", "gusto"].includes(beanItem.category))) throw new Error("사용할 원두를 선택해 주세요.");
		if (milkItem && milkQuantity > Number(milkItem.quantity)) throw new Error(`우유 재고가 부족합니다. 현재 ${milkItem.quantity}${milkItem.unit}입니다.`);
		if (beanItem && beanQuantity > Number(beanItem.quantity)) throw new Error(`${beanItem.name} 재고가 부족합니다. 현재 ${beanItem.quantity}${beanItem.unit}입니다.`);
		const statements = [];
		if (milkItem && milkQuantity > 0) statements.push(db.prepare("UPDATE inventory_items SET quantity = quantity - ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(milkQuantity, milkItem.id), db.prepare(`INSERT INTO inventory_movements
              (item_id, movement_type, quantity, movement_date, note, class_name, created_by)
             VALUES (?, 'out', ?, ?, ?, ?, ?)`).bind(milkItem.id, -milkQuantity, movementDate, note, className, user.id));
		if (beanItem && beanQuantity > 0) statements.push(db.prepare("UPDATE inventory_items SET quantity = quantity - ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(beanQuantity, beanItem.id), db.prepare(`INSERT INTO inventory_movements
              (item_id, movement_type, quantity, movement_date, note, class_name, created_by)
             VALUES (?, 'out', ?, ?, ?, ?, ?)`).bind(beanItem.id, -beanQuantity, movementDate, note, className, user.id));
		await db.batch(statements);
		await audit(user.id, "class_consumption", "inventory_movement", "", `${className} · 우유 ${milkQuantity} · 원두 ${beanQuantity}`);
...
//#endregion
//#region app/api/inventory/milk-purchase/route.ts
...
async function POST$3(request) {
	...
	try {
		...
		if (!(receipt instanceof File) || receipt.size === 0) throw new Error("우유 구매 영수증 사진을 첨부해 주세요.");
		if (!allowedTypes.has(receipt.type)) throw new Error("JPG, PNG, WebP 영수증만 등록할 수 있습니다.");
		if (receipt.size > 1e6) throw new Error("최적화된 영수증 파일은 1MB 이하여야 합니다.");
		const db = getD1();
		const milk = await db.prepare("SELECT id, name FROM inventory_items WHERE category = 'milk' AND active = 1 LIMIT 1").first();
		...
		await getReceiptsBucket().put(receiptKey, await receipt.arrayBuffer(), {
		...
		const [movementResult] = await db.batch([
         ...
         VALUES (?, 'in', ?, ?, ?, ?, ?, ?)`).bind(milk.id, quantity, movementDate, note, amount, receiptKey, user.id),
			db.prepare("UPDATE inventory_items SET quantity = quantity + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(quantity, milk.id),
           ...
           VALUES ('expense', '우유', ?, ?, ?, last_insert_rowid(), ?)`).bind(amount, movementDate, note || `${quantity}팩 구매`, user.id)
		]);
...
//#endregion
//#region app/api/inventory/roast/route.ts
...
async function POST$2(request) {
	try {
		...
		const note = optionalText(payload.note, 300);
		const db = getD1();
		const items = await db.prepare("SELECT id, name, category, quantity, unit FROM inventory_items WHERE id IN (?, ?) AND active = 1").bind(greenItemId, roastedItemId).all();
		...
		await db.batch([
			db.prepare("UPDATE inventory_items SET quantity = quantity - ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(greenKg, green.id),
			db.prepare("UPDATE inventory_items SET quantity = quantity + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(outputGrams, roasted.id),
			db.prepare(`INSERT INTO inventory_movements
            (item_id, movement_type, quantity, movement_date, note, created_by)
           VALUES (?, 'roast_out', ?, ?, ?, ?)`).bind(green.id, -greenKg, movementDate, note, user.id),
			db.prepare(`INSERT INTO inventory_movements
            (item_id, movement_type, quantity, movement_date, note, created_by)
           VALUES (?, 'roast_in', ?, ?, ?, ?)`).bind(roasted.id, outputGrams, movementDate, note, user.id)
		]);
...
//#endregion
//#region lib/roasting.ts
function parseRoastProfile(payload) {
	...
	if (!(yellowingSeconds < firstCrackSeconds && firstCrackSeconds < totalSeconds)) throw new Error("옐로잉, 1차 크랙, 배출 시간의 순서를 확인해 주세요.");
	...
	if (points[0].seconds !== 0 || points.at(-1)?.seconds !== totalSeconds) throw new Error("그래프의 첫 포인트는 0초, 마지막 포인트는 총 로스팅 시간이어야 합니다.");
	if (new Set(points.map((point) => point.seconds)).size !== points.length) throw new Error("그래프 포인트 시간은 중복될 수 없습니다.");
	if (points.some((point) => point.seconds > totalSeconds || point.gasPressure > 100)) throw new Error("그래프 시간 또는 가스 압력 범위를 확인해 주세요.");
...
function calculateRorMetrics(points, yellowingSeconds, firstCrackSeconds, totalSeconds) {
	return {
...
function averageRor(points, start, end) {
...
function interpolateTemperature(points, seconds) {
...
//#endregion
//#region app/api/roasting/route.ts
...
async function GET$2(request) {
	try {
		...
		const db = getD1();
		const profiles = await db.prepare(`SELECT p.id, p.bean_name AS beanName, p.origin, p.process,
                p.batch_weight AS batchWeight, p.charge_temp AS chargeTemp,
                p.yellowing_seconds AS yellowingSeconds,
                p.first_crack_seconds AS firstCrackSeconds,
                p.drop_temp AS dropTemp, p.total_seconds AS totalSeconds,
		...
		const points = await db.prepare(`SELECT profile_id AS profileId, seconds, bean_temp AS beanTemp, gas_pressure AS gasPressure
         FROM roasting_points ORDER BY profile_id, seconds`).all();
		...
		const result = profiles.results.map((profile) => {
			...
			return {
				...
				ror: profilePoints.length >= 2 ? calculateRorMetrics(profilePoints, Number(profile.yellowingSeconds), Number(profile.firstCrackSeconds), Number(profile.totalSeconds)) : {
					drying: 0,
					maillard: 0,
					development: 0
				}
...
async function POST$1(request) {
	try {
		...
		const profile = parseRoastProfile(await request.json());
		const db = getD1();
		const insertProfile = db.prepare(`INSERT INTO roasting_profiles
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).bind(profile.beanName, profile.origin, profile.process, profile.batchWeight, profile.chargeTemp, profile.yellowingSeconds, profile.firstCrackSeconds, profile.dropTemp, profile.totalSeconds, profile.developmentSeconds, profile.developmentRatio, profile.gasNotes, profile.notes, user.id);
		const results = await db.batch([insertProfile, ...profile.points.map((point) => db.prepare(`INSERT INTO roasting_points (profile_id, seconds, bean_temp, gas_pressure)
             SELECT seq, ?, ?, ? FROM sqlite_sequence WHERE name = 'roasting_profiles'`).bind(point.seconds, point.beanTemp, point.gasPressure))]);
...
async function PATCH$1(request) {
	try {
		...
		const db = getD1();
		if (!await db.prepare("SELECT id FROM roasting_profiles WHERE id = ?").bind(id).first()) return Response.json({ error: "수정할 로스팅 프로파일을 찾을 수 없습니다." }, { status: 404 });
		await db.batch([
			db.prepare(`UPDATE roasting_profiles
           ...
           WHERE id = ?`).bind(profile.beanName, profile.origin, profile.process, profile.batchWeight, profile.chargeTemp, profile.yellowingSeconds, profile.firstCrackSeconds, profile.dropTemp, profile.totalSeconds, profile.developmentSeconds, profile.developmentRatio, profile.gasNotes, profile.notes, id),
			db.prepare("DELETE FROM roasting_points WHERE profile_id = ?").bind(id),
			...profile.points.map((point) => db.prepare("INSERT INTO roasting_points (profile_id, seconds, bean_temp, gas_pressure) VALUES (?, ?, ?, ?)").bind(id, point.seconds, point.beanTemp, point.gasPressure))
		]);
...
async function GET$1(request) {
	try {
		...
		const staff = await getD1().prepare(`SELECT id, name, phone_last4 AS phoneLast4, role,
         ...
         FROM staff ORDER BY active DESC, role, name`).all();
		const audits = await getD1().prepare(`SELECT a.id, a.action, a.entity_type AS entityType, a.detail,
...
async function POST(request) {
	try {
		...
		const result = await getD1().prepare(`INSERT INTO staff
          (name, phone_hash, phone_last4, role, can_finance, can_inventory, can_roasting)
         VALUES (?, ?, ?, ?, ?, ?, ?)`).bind(name, await phoneHash(phone), phone.slice(-4), role, canFinance, canInventory, canRoasting).run();
		const id = Number(result.meta.last_row_id);
		await audit(actor.id, "create_staff", "staff", String(id), `${name} · ${role} · 매출 ${canFinance} · 재고 ${canInventory} · 로스팅 ${canRoasting}`);
		return Response.json({ id }, { status: 201 });
	} catch (error) {
		if ((error instanceof Error ? error.message : "").includes("UNIQUE")) return Response.json({ error: "이미 등록된 휴대폰 번호입니다." }, { status: 409 });
		return jsonError(error);
	}
}
async function PATCH(request) {
	try {
		...
		if (!allowedRoles.includes(role)) throw new Error("권한을 선택해 주세요.");
		if (id === actor.id && (!active || role !== "admin")) throw new Error("현재 로그인한 관리자 본인의 권한은 낮추거나 비활성화할 수 없습니다.");
		const db = getD1();
		const target = await db.prepare("SELECT role, active FROM staff WHERE id = ?").bind(id).first();
		if (!target) return Response.json({ error: "변경할 직원을 찾을 수 없습니다." }, { status: 404 });
		if (target.role === "admin" && Boolean(target.active) && (!active || role !== "admin")) {
			const administrators = await db.prepare("SELECT COUNT(*) AS count FROM staff WHERE role = 'admin' AND active = 1").first();
			if (Number(administrators?.count ?? 0) <= 1) throw new Error("마지막 활성 관리자의 권한은 낮추거나 비활성화할 수 없습니다.");
		}
		await db.prepare(`UPDATE staff
         SET role = ?, can_finance = ?, can_inventory = ?, can_roasting = ?,
             active = ?, updated_at = CURRENT_TIMESTAMP
         WHERE id = ?`).bind(role, canFinance, canInventory, canRoasting, active, id).run();
		await audit(actor.id, "update_staff", "staff", String(id), `${role} · 매출 ${canFinance} · 재고 ${canInventory} · 로스팅 ${canRoasting} · active=${active}`);
		return Response.json({ ok: true });
	} catch (error) {
		return jsonError(error);
	}
...
//#endregion
//#region app/api/receipts/[id]/route.ts
...
async function GET(request, context) {
	try {
		...
		const movement = await getD1().prepare("SELECT receipt_key AS receiptKey, created_by AS createdBy FROM inventory_movements WHERE id = ?").bind(movementId).first();
		if (!movement?.receiptKey) return Response.json({ error: "영수증을 찾을 수 없습니다." }, { status: 404 });
		if (movement.createdBy !== user.id && !user.canInventory && !user.canFinance) return Response.json({ error: "본인이 등록한 영수증만 볼 수 있습니다." }, { status: 403 });
...
//#endregion
//#region app/api/roasting/[id]/route.ts
var route_exports = /* @__PURE__ */ __exportAll({ DELETE: () => DELETE });
async function DELETE(request, context) {
	try {
		...
		const db = getD1();
		if (!await db.prepare("SELECT id FROM roasting_profiles WHERE id = ?").bind(id).first()) return Response.json({ error: "삭제할 로스팅 프로파일을 찾을 수 없습니다." }, { status: 404 });
		await db.batch([db.prepare("DELETE FROM roasting_points WHERE profile_id = ?").bind(id), db.prepare("DELETE FROM roasting_profiles WHERE id = ?").bind(id)]);
...
var routes = [
	...
	},
	{
		...
		ids: {
			"route": "route:/api/auth/bootstrap",
			"page": null,
			"routeHandler": "route-handler:/api/auth/bootstrap",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/auth/login",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/auth/logout",
			"page": null,
			"routeHandler": "route-handler:/api/auth/logout",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/auth/status",
			"page": null,
			"routeHandler": "route-handler:/api/auth/status",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			...
			"page": null,
			"routeHandler": "route-handler:/api/dashboard",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/finance",
			"page": null,
			"routeHandler": "route-handler:/api/finance",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/inventory",
			"page": null,
			"routeHandler": "route-handler:/api/inventory",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/inventory/class-use",
			"page": null,
			"routeHandler": "route-handler:/api/inventory/class-use",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/inventory/class-use",
		patternParts: [
			"api",
			"inventory",
			"class-use"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/inventory/milk-purchase",
			"page": null,
			"routeHandler": "route-handler:/api/inventory/milk-purchase",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/inventory/milk-purchase",
		patternParts: [
			"api",
			"inventory",
			"milk-purchase"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/inventory/roast",
			"page": null,
			"routeHandler": "route-handler:/api/inventory/roast",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/inventory/roast",
		patternParts: [
			"api",
			"inventory",
			"roast"
	...
	},
	{
		...
		ids: {
			"route": "route:/api/roasting",
			"page": null,
			"routeHandler": "route-handler:/api/roasting",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/roasting",
		patternParts: ["api", "roasting"],
		isDynamic: false,
		params: [],
		rootParamNames: [],
		page: null,
	...
	},
	{
		...
		ids: {
			"route": "route:/api/staff",
			"page": null,
			"routeHandler": "route-handler:/api/staff",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/receipts/:id",
			"page": null,
			"routeHandler": "route-handler:/api/receipts/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
	...
	},
	{
		...
		ids: {
			"route": "route:/api/roasting/:id",
			"page": null,
			"routeHandler": "route-handler:/api/roasting/:id",
			"rootBoundary": "root-boundary:/",
			"layouts": ["layout:/"],
			"templates": [],
			"slots": {}
		},
		pattern: "/api/roasting/:id",
		patternParts: [
			"api",
			"roasting",
			":id"
...
var __publicFiles = new Set([
	"/brand/monthly-coffee.png",
	"/brand/thecup-edu.jpg",
	"/og.png"
]);
